#!/usr/bin/env python
'''
This code uses the Hokuyo UST-10LX Lidar data obtained thru the racecar package thru scomo.launch to drive the car along a wall to its left.
...
def goBack(init_front_dist, rev_dist, rate, ads_msg, fetcher, publisher):
    ...
    while (front_dist_now < init_front_dist + rev_dist) and not rospy.is_shutdown():
        print("Going back")
...
def main():
    rospy.init_node("HallwayNavigation")
    rate = rospy.Rate(40)
